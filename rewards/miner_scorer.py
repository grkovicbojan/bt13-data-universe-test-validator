...
class MinerScorer:
    ...
    def compute_weight_components(
        ...
        """Return capped-score components for one miner (used by dashboard metrics)."""
        with self.lock:
            return self._compute_weight_components_unlocked(uid, relax_service_cap)

    def _compute_weight_components_unlocked(
        self, uid: int, relax_service_cap: bool = False
    ) -> dict:
        ...
        p2p_component = (
            float(self.scorable_bytes[uid]) * MinerScorer.P2P_REWARD_SCALE * p2p_cred
        )
        s3_component = float(self.s3_boosts[uid]) * s3_cred
        od_component = float(self.ondemand_boosts[uid]) * od_cred

        if relax_service_cap:
            p2p_capped = p2p_component
            capped_score = p2p_capped + s3_component + od_component
        else:
            ...
            p2p_capped = (
                min(p2p_component, service_component) if service_component > 0 else 0.0
            )
            capped_score = p2p_capped + s3_component + od_component

        return {
            "p2p_component": p2p_capped,
            "s3_component": s3_component,
            "od_component": od_component,
            "capped_score": capped_score,
        }

    def get_scores_for_weights(self, relax_service_cap: bool = False) -> torch.Tensor:
        """Returns scores with P2P capped at (S3 + OD) for weight setting.

        When ``relax_service_cap`` is True (local testnet dashboard), P2P counts
        without requiring S3/OD service contribution.
        """
        with self.lock:
            capped_scores = torch.zeros_like(self.scores)

            for uid in range(len(self.scores)):
                components = self._compute_weight_components_unlocked(
                    uid, relax_service_cap
                )
                capped_scores[uid] = components["capped_score"]
    ...
    def reset_all(self) -> None:
        """Resets scores and credibility for every neuron slot."""
        with self.lock:
            self.scores.zero_()

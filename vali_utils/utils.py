...
def choose_data_entity_bucket_to_query(
...
) -> Optional[DataEntityBucket]:
    """Securely pick one DataEntityBucket to validate.

    Uses a seed based on system time. Returns None when the index has no
    buckets or no scorable bytes (common on testnet when DD weights zero out
    all P2P buckets).
    """
    buckets = list(index.scorable_data_entity_buckets or [])
    if not buckets:
        return None

    positive = [b for b in buckets if (b.scorable_bytes or 0) > 0]
    candidates = positive if positive else buckets
    total_size = sum(b.scorable_bytes or 0 for b in candidates)

    if total_size <= 0:
        return random.choice(candidates).to_data_entity_bucket()
    ...
    iterated_bytes = 0
    for scorable_bucket in candidates:
        size = scorable_bucket.scorable_bytes or 0
        if iterated_bytes + size >= chosen_byte:
            return scorable_bucket.to_data_entity_bucket()
        iterated_bytes += size

    # Floating-point edge case: fall back to the last candidate.
    return candidates[-1].to_data_entity_bucket()

def choose_entities_to_verify(
    entities: List[DataEntity],
    mode: str = "sample",
) -> List[DataEntity]:
    """Given a list of DataEntities from a DataEntityBucket, chooses entities to verify.

    mode "full" validates every entity in the bucket with the scraper.
    mode "sample" (default) picks 1–2 entities weighted by content size.
    """
    if mode == "full":

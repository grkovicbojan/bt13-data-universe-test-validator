...
class OnDemandValidator:
    ...
    async def build_scraper_failure_comparison(
        self, ctx: ValidationContext, entity: DataEntity
    ) -> Dict[str, Any]:
        """Build miner vs validator comparison payload for dashboard failures."""
        from vali_utils.dashboard.validation_reports import (
            enrich_content_comparison,
            preview_entity,
        )

        platform = ctx.source.upper()
        comparison: Dict[str, Any] = {
            "uri": entity.uri,
            "platform": platform.lower(),
            "miner_submission": preview_entity(entity),
            "validator_fetched": None,
            "validator_message": "",
            "is_valid": False,
        }

        # Fetch live validator content first so the dashboard can show it even
        # when scraper.validate() only returns a reason string.
        if platform == "REDDIT":
            fetched = await self._fetch_reddit_validator_preview(entity)
            if fetched:
                comparison["validator_fetched"] = fetched
        elif platform == "X":
            fetched = await self._fetch_x_validator_preview(entity)
            if fetched:
                comparison["validator_fetched"] = fetched
            ...
            try:
                if platform == "X":
                    results = await scraper.validate(
                        entities=[entity], allow_low_engagement=True
                    )
                else:
                    results = await scraper.validate([entity])
                if results:
                    comparison["is_valid"] = bool(results[0].is_valid)
                    comparison["validator_message"] = (
                        results[0].reason or "Scraper validation failed"
                    )
            except Exception as e:
                comparison["validator_message"] = f"Scraper error: {e}"

        if platform == "REDDIT" and not comparison.get("validator_fetched"):
            fetched = await self._fetch_reddit_validator_preview(entity)
            if fetched:
                comparison["validator_fetched"] = fetched
        elif platform == "X" and not comparison.get("validator_fetched"):
            fetched = await self._fetch_x_validator_preview(entity)
            if fetched:
                comparison["validator_fetched"] = fetched

        return enrich_content_comparison(comparison)

    async def _fetch_x_validator_preview(
        self, entity: DataEntity
    ) -> Optional[Dict[str, Any]]:
        """Re-fetch X content the validator uses for comparison."""
        from vali_utils.dashboard.validation_reports import preview_entity

        scraper = self._get_scraper("X")
        if not scraper:
            return None
        ...
        try:
            live = None
            fetch_live = getattr(scraper, "fetch_live_content_from_url", None)
            if callable(fetch_live):
                live = await fetch_live(ent_content.url, allow_low_engagement=True)
            if not live:
                return None
            actual_entity = XContent.to_data_entity(content=live)
            return preview_entity(actual_entity)
        except Exception as e:
            bt.logging.debug(f"X validator preview fetch failed: {e}")
            return None

    async def validate_entity_with_messages(
        self,
        ctx: ValidationContext,
        entity: DataEntity,
        post_id: str,
        ...
        """Run entity validation and return ordered failure messages."""
        messages: List[Dict[str, str]] = []

        if not self._validate_request_fields(ctx, entity, miner_uid):
            msg = (
                "Post failed request field validation "
                "(username/keyword/date/URL mismatch)."
            )
            messages.append({"phase": "request_fields", "message": msg})
            return {
                "passed": False,
                "phase": "request_fields",
                "messages": messages,
                "uri": entity.uri,
            }

        if not self._validate_metadata_completeness(entity, post_id, miner_uid):
            msg = "Post failed metadata completeness validation."
            messages.append({"phase": "metadata", "message": msg})
            return {
                "passed": False,
                "phase": "metadata",
                "messages": messages,
        ...
        comparison = await self.build_scraper_failure_comparison(
            ctx, entity_for_validation
        )
        if not comparison.get("is_valid"):
            scraper_msg = (
                comparison.get("validator_message")
                or "Scraper validation failed."
            )
            messages.append({"phase": "scraper", "message": scraper_msg})
            return {
                "passed": False,
                "phase": "scraper",
                "messages": messages,
        ...
        return {"passed": True, "messages": [], "uri": entity.uri}

    async def _fetch_reddit_validator_preview(
        self, entity: DataEntity
    ) -> Optional[Dict[str, Any]]:
        """Re-fetch Reddit content the validator uses for comparison."""
        from scraping.proxy import proxy_client_session
        from vali_utils.dashboard.validation_reports import preview_entity
        ...
        scraper = self._get_scraper("reddit")
        if not scraper:
            return None
        ...
        try:
            ...
            fetch_live = getattr(scraper, "fetch_live_content_from_url", None)
            if callable(fetch_live):
                live = await fetch_live(ent_content.url)
            elif hasattr(scraper, "_fetch_content_from_url"):
                session_proxy = scraper._pick_proxy_for_session()
                with scraper._track_proxy_session(session_proxy):
                    async with proxy_client_session(
                        session_proxy,
                        getattr(scraper, "USER_AGENT", "validator"),
                        getattr(scraper, "REQUEST_TIMEOUT", 30),
                    ) as session:
                        data_type = getattr(ent_content, "data_type", None)
                        try:
                            live = await scraper._fetch_content_from_url(
                                session, ent_content.url, data_type
                            )
                        except TypeError:
                            live = await scraper._fetch_content_from_url(
                                session,
                                ent_content.url,
                                data_type,
                                getattr(ent_content, "id", None),
                            )

            if not live:
                return None
            actual_entity = RedditContent.to_data_entity(live)
            return preview_entity(actual_entity)
        except Exception as e:
            bt.logging.debug(f"Reddit validator preview fetch failed: {e}")
            return None

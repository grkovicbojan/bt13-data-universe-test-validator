...
PREFERRED_SCRAPERS = {
    ...
    DataSource.REDDIT: ScraperId.REDDIT_DOM,
}
...
class DuckDBSampledValidator:
    ...
    async def validate_miner_s3_data(
        ...
        try:
            ...
            duckdb_samples_per_file = 500 if self.full_validation else 100
            duckdb_result = await self._sampled_duckdb_validation(
                sampled_files,
                expected_jobs,
                presigned_urls,
                samples_per_file=duckdb_samples_per_file,
            )
            ...
            if duckdb_result.get("reason"):
                ...
                job_match_samples = 100 if self.full_validation else 10
                job_match_result = await self._perform_job_content_matching(
                    sampled_files,
                    expected_jobs,
                    presigned_urls,
                    samples_per_file=job_match_samples,
                )
                ...
                if older_files:
                    if self.full_validation:
                        scraper_files.extend(older_files)
                    else:
                        older_sample = random.sample(
                            older_files, min(len(older_files), max(5, len(recent_files)))
                        )
                ...
                if scraper_files:
                    ...
                    scraper_entity_budget = 10000 if self.full_validation else 20
                    scraper_result = await self._perform_scraper_validation(
                        miner_hotkey,
                        scraper_files,
                        expected_jobs,
                        presigned_urls,
                        num_entities=scraper_entity_budget,
                    )
                else:
                    bt.logging.info(
    ...
    async def _sampled_duckdb_validation(
        ...
        if self.full_validation:
            files_to_check = list(sampled_files)
        else:
    ...
    async def _perform_scraper_validation(
        ...
        entity_budget = num_entities * 2 if not self.full_validation else num_entities
        MIN_ROWS_PER_FILE = 2
        MAX_ROWS_PER_FILE = 500 if self.full_validation else 20
        ...
        for file_info in sampled_files:
            ...
            if not self.full_validation and len(all_entities) >= num_entities * 2:
                break
        ...
        if self.full_validation:
            entities_to_validate = list(all_entities)
        else:

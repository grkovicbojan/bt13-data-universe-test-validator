...
class ListActiveJobsRequest(BaseModel):
    ...
    created_since: Optional[datetime] = Field(
        default=None,
        description="If set, only jobs created at or after this time are returned.",
    )
...
class DataUniverseApiClient:
    ...
    async def miner_submit_and_upload(
        ...
        if not presigned or "url" not in presigned:
            raise RuntimeError(
                "Server did not return a valid presigned_post_upload_data (missing url)."
            )

        client = self._ensure_client()
        upload_url = presigned["url"]
        method = (presigned.get("method") or "POST").upper()
        payload_json = data.model_dump_json()

        if method == "PUT":
            # Local testnet API uses raw PUT uploads instead of S3 multipart POST.
            put_resp = await client.put(
                upload_url,
                content=payload_json,
                headers={"Content-Type": "application/json"},
            )
            if put_resp.status_code not in (204, 201, 200):
                raise RuntimeError(
                    f"Upload failed: {put_resp.status_code} {put_resp.text}"
                )
        else:
            if "fields" not in presigned:
                raise RuntimeError(
                    "Server did not return presigned fields for POST upload."
                )
            ...
            files = {
                "file": ("data.json", payload_json, content_type),
            }

            post_resp = await client.post(upload_url, data=fields, files=files)
        ...
        # bt.logging.info(f"---Success Uploaded data for job {job_id} to {presigned['url']}, {presigned["fields"]}, {post_resp.status_code}, {data.model_dump_json()}, {post_resp.headers}")
